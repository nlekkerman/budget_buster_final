...
let isSearchStopped = false; // Flag to track if the search has been stopped


function captureFrameAndRecognize() {
    if (!shouldRecognize || isSearchStopped) {
        return;
    }
    ...
    Tesseract.recognize(
        ...
        const foundNumbers = findAllNumbers(text, /\b\d+\.\d+\b/g);
        if (foundNumbers.length > 0) {
            ...
            if (biggestNumber === null || currentBiggest.length > biggestNumber.length) {
                ...
                // Stop the search (recognition) when a number is found
                stopSearch();
                return;
            }
        }
        ...
        // Optionally, you can reset the recognition state or take other actions
    });
}

// Function to stop the search (recognition)
function stopSearch() {
    isSearchStopped = true;

    // Optionally, you can add additional logic or UI updates when the search is stopped
    // For example, you can display a message to the user indicating that the search has stopped
}
    ...
    function populateShoppingListLists() {
        ...
        existingLists.forEach(list => {
            ...
            listItem.addEventListener('click', function () {
                ...
                // Populate camera-shopping-list with items from the selected list
                list.items.forEach(item => {
                    ...
                    itemElement.addEventListener('click', function () {
                        ...
                        // Add your logic here for handling the click on each item
                    });
